...
def ae_gpu_worker(gpu_id, chunk_indices, model_config, train_config,
    ...
    def setup_fn(device):
        ...
        ctx = dict(buffers=buffers, train_kernel=train_kernel,
                   batch_encode=batch_encode, ridge_eye=ridge_eye,
                   n_components=n_components, param_keys=param_keys,
    ...
    def chunk_fn(ctx, current_params, X_chunk, y_chunk, X_test_chunk,
        ...
        with torch.no_grad():
            Z_train = ctx['batch_encode'](trained_params, ctx['buffers'],
                                          X_chunk)
            n = ctx['n_components']
